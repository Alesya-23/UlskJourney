...
class SignInFragment : Fragment(R.layout.sign_in_fragment) {
    ...
    private fun signIn() {
        ...
        if (checkUser() && isDataInCorrect) {
            ...
            Toast.makeText(activity?.applicationContext, "Пользователь не найден. Зарегистрируйтесь", Toast.LENGTH_LONG).show()
        }
    }
    ...
    private fun checkDataSignIn(login: String, password: String): Boolean {
            ...
            rootRef.addListenerForSingleValueEvent(object : ValueEventListener {
                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.child(login).exists()) {
                        val loginDB = snapshot.child(login).child("login").toString()
                        val passwordDB = snapshot.child(login).child("password").toString()
